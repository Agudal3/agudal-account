    ...
    */
    var abstractReconciliation = Widget.extend(ControlPanelMixin, {
                ...
                deferred_promises.push(new Model("account.account")
                    ...
                    .all().then(function (data) {
                        _.each(data, function (o) {
                            self.map_account_id_code[o.id] = o.code
                        });
                    })
                );
        ...
        renderPresetsButtons: function (presets) {
            ...
            presets_array = _.sortBy(presets_array, function (o) {
                return o.sequence
            });
        ...
        keyboardShortcutsHandler: function (e) {
            ...
            if ((e.which === 13 || e.which === 10) && (e.ctrlKey || e.metaKey)) {
                self.processReconciliations(_.filter(self.getChildren(), function (o) {
                    return self.monetaryIsZero(o.get("balance"));
                }));
            }
        },
    ...
    var abstractReconciliationLine = Widget.extend({
            ...
            return self._super().then(function () {
                ...
                return $.when(self.loadData()).then(function () {
                    return $.when(self.render()).then(function () {
                        ...
                        if (self.context.animate_entrance) {
                            return self.$el.stop(true, true).fadeIn({
                                duration: self.aestetic_animation_speed,
                                queue: false
                            }).css('display', 'none').slideDown(self.aestetic_animation_speed);
                        }
                    });
                });
            });
        ...
        createFormWidgets: function () {
            ...
            for (var i = 0; i < create_form_fields_arr.length; i++) {
                ...
                // append to DOM
                var $field_container = $(QWeb.render("form_create_field", {
                    id: field_data.id,
        ...
        selectMoveLine: function (mv_line) {
            ...
            if (!line) {
                ...
                self.mv_lines_deselected = _.filter(self.mv_lines_deselected, function (o) {
                    return o.id != line_id
                });
            }
        ...
        deselectMoveLine: function (mv_line) {
            var self = this;
            var line_id = mv_line.dataset.lineid;
            var line = _.find(self.get("mv_lines_selected"), function (o) {
                return o.id == line_id
            });
            if (!line) return; // If no line found, we've got a syncing problem (let's turn a deaf ear)
            ...
            self.set("mv_lines_selected", _.filter(self.get("mv_lines_selected"), function (o) {
                return o.id != line_id
            }));
        ...
        pagerControlLeftHandler: function () {
            var self = this;
            if (self.$(".pager_control_left").hasClass("disabled")) {
        ...
        pagerControlRightHandler: function () {
            ...
            if (self.$(".pager_control_right").hasClass("disabled")) {
                return; /* shouldn't happen, anyway*/
            }
        ...
        removeLine: function ($line) {
            ...
            self.set("lines_created", _.filter(self.get("lines_created"), function (o) {
                return o.id != line_id
            }));
            self.amount_field.set("value", -1 * self.get("balance"));
        },
        ...
        updateAccountingViewMatchedLines: function () {
            var self = this;
            $.each(self.$(".tbody_matched_lines .bootstrap_popover"), function () {
        ...
        updateAccountingViewCreatedLines: function () {
            var self = this;
            $.each(self.$(".tbody_created_lines .bootstrap_popover"), function () {
                $(this).popover('destroy')
            });
        ...
        updateMatchView: function () {
            ...
            // Display move lines
            var template_name = (QWeb.has_template(this.template_prefix + "reconciliation_move_line") ? this.template_prefix : "") + "reconciliation_move_line";
            $.each(self.$(".match table .bootstrap_popover"), function () {
                $(this).popover('destroy')
            });
            table.empty();
            ...
            _(_.filter(self.mv_lines_deselected, function (o) {
                ...
                try {
                    floatFromFilter = parse_value(self.filter, {type: 'float'});
                } catch (e) {
                }
        ...
        updateMatches: function () {
            ...
            var excluded_ids = _.collect(self.get("mv_lines_selected").concat(self.mv_lines_deselected), function (o) {
                return o.id;
            });
        ...
        bowOut: function (speed, doPostMortemProcess) {
            ...
            return $.when(self.$el.parent().animate({
                height: 0,
                marginBottom: 0
            }, speed * height / 150)).then(function () {
                $.each(self.$(".bootstrap_popover"), function () {
                    $(this).popover('destroy')
                });
                _.each(self.getChildren(), function (o) {
                    o.destroy()
                });
    ...
    var bankStatementReconciliation = abstractReconciliation.extend({
                ...
                return $.when.apply($, deferred_promises).then(function () {
                    ...
                    self.$el.find('.js_automatic_reconciliation').click(function () {
                        ...
                        self.model_bank_statement_line
                            ...
                            .then(function () {
                                self.$('.js_automatic_reconciliation').hide();
        ...
        processReconciliations: function (reconciliations) {
            ...
            var self = this;
            var ids = _.collect(reconciliations, function (o) {
                return o.line_id
            });
            var data = _.collect(reconciliations, function (o) {
                return o.prepareDataForPersisting()
            });
        ...
        excludeMoveLines: function (source_child, partner_id, lines) {
            var self = this;
            var line_ids = _.collect(lines, function (o) {
                return o.id
            });
            ...
            _.each(self.getChildren(), function (child) {
                ...
                if (child.partner_id === partner_id || child.st_line.has_no_partner) {
                    if (contains_lines(child.get("mv_lines_selected"), line_ids)) {
                        child.set("mv_lines_selected", _.filter(child.get("mv_lines_selected"), function (o) {
                            return line_ids.indexOf(o.id) === -1
                        }));
                    } else if (contains_lines(child.mv_lines_deselected, line_ids)) {
                        child.mv_lines_deselected = _.filter(child.mv_lines_deselected, function (o) {
                            return line_ids.indexOf(o.id) === -1
                        });
                        child.updateMatches();
        ...
        displayDoneMessage: function (all_was_auto_reconciled) {
            ...
            if (self.$(".button_close_statement").length !== 0) {
                ...
                self.model_bank_statement
                    ...
                    .all()
                    .then(function (data) {
                        if (_.all(data, function (o) {
                            return self.monetaryIsZero(o.balance_end_real - o.balance_end)
                        })) {
    ...
    var bankStatementReconciliationLine = abstractReconciliationLine.extend({
        ...
        selectMoveLine: function (mv_line) {
            ...
            var selected_lines_account_types = _.collect(self.get("mv_lines_selected").concat(line), function (l) {
        ...
        doPartialReconcileButtonClickHandler: function (e) {
            ...
            var line = _.find(this.get("mv_lines_selected"), function (o) {
                return o.id == line_id
            });
        ...
        updateMatchesGetMvLines: function (excluded_ids, offset, limit, callback) {
            ...
            if (self.st_line.has_no_partner)
                _.each(self.getParent().excluded_move_lines_ids, function (o) {
                    globally_excluded_ids = globally_excluded_ids.concat(o)
                });
            else
        ...
        prepareDataForPersisting: function () {
            var self = this;
            var counterpart_aml = _.filter(this.get("mv_lines_selected"), function (line) {
                return !line.already_paid
            });
            var counterpart_aml_dicts = _.collect(counterpart_aml, function (line) {
                return self.prepareCounterpartAMLDict(line)
            });
            var payment_aml = _.filter(this.get("mv_lines_selected"), function (line) {
                return line.already_paid
            });
            var payment_aml_ids = _.collect(payment_aml, function (line) {
                return line.id
            });
    ...
    var manualReconciliation = abstractReconciliation.extend({
            ...
            return $.when(this._super()).then(function () {
                return self.model_aml.call("get_data_for_manual_reconciliation_widget", [self.partner_ids, self.account_ids]).then(function (data) {
                    ...
                    if (self.working_on_a_given_item) {
                        ...
                        if (data_customers_len === 1) {
                            self.title += data.customers[0].partner_name
                        } else if (data_suppliers_len === 1) {
                            self.title += data.suppliers[0].partner_name
                        } else if (data_accounts_len === 1) {
                            self.title += data.accounts[0].account_code
                        }
                    }
                    ...
                    if (data_customers_len !== 0) {
                        ...
                        self.customers_data.items = _.each(data.customers, function (item) {
                            self.prepareReconciliationData(item, 'customer')
                        });
                    }
                    if (data_suppliers_len !== 0) {
                        self.suppliers_data.num_total = data_suppliers_len;
                        self.suppliers_data.items = _.each(data.suppliers, function (item) {
                            self.prepareReconciliationData(item, 'supplier')
                        });
                    }
                    if (data_accounts_len !== 0) {
                        self.accounts_data.num_total = data_accounts_len;
                        self.accounts_data.items = _.each(data.accounts, function (item) {
                            self.prepareReconciliationData(item, 'account')
                        });
                    }
        ...
        processReconciliations: function (reconciliations) {
            if (reconciliations.length === 0) return;
            var self = this;
            var reconciliations_type_count = _.countBy(reconciliations, function (rec) {
                return rec.data.reconciliation_type;
            });
            var data = _.collect(reconciliations, function (rec) {
                return {
                    ...
                    mv_line_ids: _.collect(rec.get("mv_lines_selected"), function (o) {
                        return o.id
                    }),
        ...
        getDisplayedCollections: function () {
            var collections = [];
            if (this.mode === 'customers' || this.mode === 'all') {
                collections.push(this.customers_data)
            }
            if (this.mode === 'suppliers' || this.mode === 'all') {
                collections.push(this.suppliers_data)
            }
            if (this.mode === 'others' || this.mode === 'all') {
                collections.push(this.accounts_data)
            }
            return collections;
        },
        ...
        updateProgress: function (animate_entrance) {
            ...
            if (self.mode !== 'all') {
                _.each(self.getChildren(), function (child) {
                    ...
                    if (type !== self.mode) {
                        num_child_displayed--;
                        child.$el.slideUp(self.aestetic_animation_speed, function () {
                            child.destroy()
                        });
            ...
            var items_to_display = _.foldl(displayed_collections, function (prev, current) {
                return prev.concat(current.items)
            }, []);
            // Filter out those which are already displayed
            items_to_display = _.filter(items_to_display, function (item) {
            ...
            var reconciliations_left = _.foldl(displayed_collections, function (prev, current) {
                return prev + current.num_total - current.num_done
            }, 0);
        ...
        updateProgressbar: function () {
            ...
            var done = _.foldl(displayed_collections, function (prev, current) {
                return prev + current.num_done
            }, 0);
            var total = _.foldl(displayed_collections, function (prev, current) {
                return prev + current.num_total
            }, 0);
    ...
    var manualReconciliationLine = abstractReconciliationLine.extend({
        ...
        mvLinesSelectedChanged: function (elt, val) {
            ...
            if (lines.length > 0) {
                ...
                var same_currency = _.every(lines, function (line) {
                    return line.currency_id === first_line_currency
                });
                if (same_currency)
        ...
        currencyChanged: function () {
            ...
            this.updateMatches();
            var line_ids = _.collect(self.get("mv_lines_selected"), function (o) {
                return o.id
            });
            self.model_aml
